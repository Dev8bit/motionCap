...
#define BATTERY_LEVEL_MEAS_INTERVAL     APP_TIMER_TICKS(20, APP_TIMER_PRESCALER) /**< Battery level measurement interval (ticks). */
...
//private defination
#define HAT_ABS_MODE					1 //HAT控制模式，1绝对式0增量式
#define THRESHOLD_MOVE					2
#define HAT_CENTER_X					468
#define HAT_CENTER_Y					472

//private variable
static bool m_bHIDAction = false;
uint8_t butStateArray[BUTTONS_NUMBER] = {0};

float xAxisPre = 0.0, yAxisPre = 0.0;
int16_t x_delta = 0, y_delta = 0;

float xAxisPreHat = 0.0, yAxisPreHat = 0.0;
int16_t x_deltaHat = 0, y_deltaHat = 0;

int8_t pointerMovementCoef = -2;
...
static void battery_level_meas_timeout_handler(void * p_context)
{
	...
	if(!m_bHIDAction) m_bHIDAction = true;
}
...
/**@brief Function for sending a button .
 *
 * @param[in]   buttonNum  buttonNum.
 * @param[in]   buttonState  buttonState.
 */
static void mouse_buttons_send(int8_t buttonStateMask)
{
    ret_code_t err_code;
...
int main(void)
{
	...
	nrf_twi_init();
	BMX055_init();
	nrf_timer_SampleBMX055_init();
	NRF_LOG_FLUSH();
	
	//set bias
	BMX055_setMagCalib(228.916f, -19.311f, 240.118f, 33.343, 226.771, 73.621);
    ...
    for (;;)
    {
		...
		nrf_SampleBMX055_loopRoutine();
		
		if(m_bHIDAction)
		{
			x_delta = getScreenAxisX() - xAxisPre;
			y_delta = getScreenAxisY() - yAxisPre;
			
			if((abs(x_delta) > THRESHOLD_MOVE) || (abs(y_delta) > THRESHOLD_MOVE))
			{
				if(x_delta > 180) x_delta -= 360;
				if(x_delta < -180) x_delta += 360;
				if(y_delta > 90) x_delta -= 180;
				if(y_delta < -90) x_delta += 180;
				
				if (m_conn_handle != BLE_CONN_HANDLE_INVALID)
				{
					mouse_movement_send(-x_delta*pointerMovementCoef, y_delta*pointerMovementCoef);
				}

				xAxisPre = getScreenAxisX();
				yAxisPre = getScreenAxisY();
			}			
			
			static int8_t butStateMask = 0;
			static int8_t butStateMaskPre = 0;
			
			for (uint8_t num = 0; num < BUTTONS_NUMBER; num++)
			{
				if(butStateArray[num] == 1)
				{
					butStateMask |= (1 << num);
					
					bool bState = false;
					bsp_button_is_pressed(num, &bState);
					if(!bState)
					{
						butStateMask &= ~(1 << num);
						butStateArray[num] = 0;						
			...
			if(butStateMask != butStateMaskPre)
			{
				if (m_conn_handle != BLE_CONN_HANDLE_INVALID)
				{
					mouse_buttons_send(butStateMask);
				}
				
				
				butStateMaskPre = butStateMask;
				
				NRF_LOG_INFO("key_Mask:");
				NRF_LOG_HEXDUMP_INFO(&butStateMask, 1);
				NRF_LOG_FLUSH();
			}
	...
	//			----|--->x		hat value
	//				|
//				NRF_LOG_INFO("Ain X:%d ", m_startUpVal_X);
//				NRF_LOG_INFO("Ain Y:%d\r\n", m_startUpVal_Y);
//				NRF_LOG_FLUSH();			

				x_deltaHat = m_startUpVal_X - xAxisPreHat;
				y_deltaHat = m_startUpVal_Y - yAxisPreHat;			
				if((abs(x_deltaHat) > THRESHOLD_MOVE) || (abs(y_deltaHat) > THRESHOLD_MOVE))
				{
					if (m_conn_handle != BLE_CONN_HANDLE_INVALID)
					{
						mouse_movement_send(x_deltaHat, -y_deltaHat);
					}
#if HAT_ABS_MODE
					xAxisPreHat = HAT_CENTER_X;
					yAxisPreHat = HAT_CENTER_Y;
#else
					xAxisPreHat = m_startUpVal_X;
					yAxisPreHat = m_startUpVal_Y;
#endif
				}		
			}
			m_bHIDAction = false;
